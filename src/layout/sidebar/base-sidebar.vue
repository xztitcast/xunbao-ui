...
const getPopClassName = () => {
  ...
  return `tt-sidebar-menu-pop-${props.mode === "vertical" && sidebarCache === "dark" ? "dark" : "light"}`;
};
...
watch(
  () => route.path,
  (vl) => {
    const matchedRoute = getValueByKeys(getValueByKeys(router.currentRoute.value, "matched", [])[0], "path", "");
