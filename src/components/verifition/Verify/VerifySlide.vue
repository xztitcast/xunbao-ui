    ...
    <div style="position: relative;">
        <div v-if="type === '2'" class="verify-img-out"
            ...
            <div class="verify-img-panel" :style="{width: setSize.imgWidth,
                ...
                <img :src=" blockBackImgBase ? 'data:image/png;base64,' + backImgBase : defaultImg" alt="" style="width:100%;height:100%;display:block">
                <div class="verify-refresh" @click="refresh" v-show="showRefresh"><i class="iconfont icon-refresh"></i>
                </div>
                <transition name="tips">
                    <span class="verify-tips" v-if="tipWords" :class="passFlag ?'suc-bg':'ett-bg'">{{tipWords}}</span>
                </transition>
            </div>
        </div>
        <!-- 公共部分 -->
            ...
            let secretKey = ref(''),           //后端返回的ase加密秘钥
                ...
                function end() {
                    ...
                    if (status.value && isEnd.value == false) {
                        ...
                        baseService
                            .post('/sys/dynamic/captcha/check', data)
                            .then(({data}) => {
                            if (data && data.code === 0) {
                ...
                function getPictrue(){
                    ...
                    baseService
                        .post('/sys/dynamic/captcha/get', data)
                        .then(({data}) => {
                        if (data && data.code === 0) {
                            backImgBase.value = data.result.originalImageBase64
                            blockBackImgBase.value = data.result.jigsawImageBase64
                            backToken.value = data.result.token
                            secretKey.value = data.result.secretKey
                        }else{
                            tipWords.value = data.message;
                        }
                    })
                }
                return {
